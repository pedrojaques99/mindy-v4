    ...
    try {
      ...
      if (error) {
        ...
        try {
          ...
          setSupabaseStatus({
            status: 'degraded',
            error: 'Connected but could not fetch table info',
            responseTime: Math.round(responseTime),
            tables: [],
            lastChecked: new Date()
          });
          ...
          <div className="glass-card p-6">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center">
                ...
                <h2 className="text-xl font-medium">Server Status</h2>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span>{serverStatus.responseTime ? `${serverStatus.responseTime}ms` : 'N/A'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-white/60">Uptime:</span>
                <span>{serverStatus.uptime} hours</span>
              </div>
              <div className="flex justify-between">
                <span className="text-white/60">Memory Usage:</span>
                <span>{serverStatus.memoryUsage || 'N/A'}</span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="glass-card p-6">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                ...
                {supabaseStatus.status.toUpperCase()}
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className="text-white/60">Response Time:</span>
                <span>{supabaseStatus.responseTime ? `${supabaseStatus.responseTime}ms` : 'N/A'}</span>
              </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-dark-300">
                    <th className="text-left pb-2 text-white/70">Table Name</th>
                    <th className="text-left pb-2 text-white/70">Rows</th>
                    <th className="text-left pb-2 text-white/70">Status</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <td className="py-2">
                        <div className="flex items-center">
                          {table.status === 'accessible' ? (
                            ...
                            <XCircleIcon className="w-4 h-4 text-red-400 mr-1.5" />
                          )}
                          <span className={table.status === 'accessible' ? 'text-green-400' : 'text-red-400'}>
                            {table.status}
                          </span>
                        </div>
                      </td>
