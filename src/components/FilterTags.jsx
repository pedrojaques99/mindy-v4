    ...
    <div className="mb-4">
      <div className="flex justify-between items-center mb-2">
        <h3 className="text-sm text-gray-400 font-medium">
          {t('home.filters.filterByTags', 'Filter by tags')}: {selectedTags.length > 0 && <span className="text-lime-accent">({selectedTags.length} {t('home.filters.selected', 'selected')})</span>}
        </h3>
        {selectedTags.length > 0 && (
          ...
          >
            ...
            <span>{t('home.filters.clear', 'Clear')}</span>
          </motion.button>
        )}
      </div>
      ...
      >
            ...
            >
              {isSoftwareTag(tag) ? (
                <span className="flex items-center gap-1">
                  ...
                  <span>{t(`software.${tag.toLowerCase()}`, tag)}</span>
                </span>
              ) : (
        ...
        {/* Empty state */}
        {tags.length === 0 && (
          <div className="text-gray-400 text-xs italic">{t('home.filters.noTags', 'No tags available')}</div>
        )}
      </motion.div>
      ...
      {selectedTags.length > 0 && (
        ...
        >
          <div className="w-full text-xs text-gray-400 mb-1">{t('home.filters.selectedFilters', 'Selected filters')}:</div>
